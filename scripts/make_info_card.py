...
ROWS = [
    ("host",),
    ("kv", "Now", "Founder @ VisualizeMyData"),
    ("kv", "Role", "Food Technologist x Developer"),
    ("kv", "Edu", "B.Tech Food Tech & Developer"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Data Vis", "Python, D3.js, React, SQL"),
    ("kv", "Food QA", "ISO 22000, HACCP standards"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Founder @ VisualizeMyData"),
    ("bul", "Expert in ISO 22000 & HACCP safety"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">satyasanatan989@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "gap":
        ...
        continue
    if kind == "host":
        username = "satyasanatan989"
        username_len = len(username + "@github")
        line_start_x = KEY_X + username_len * 8.4 + 10
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">{username}</tspan><tspan fill="{MUTED}">@</tspan>'
                 f'<tspan fill="{ACCENT}">github</tspan></text>'
                 f'<line x1="{line_start_x:.1f}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
